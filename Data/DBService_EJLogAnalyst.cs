...
namespace SLA_Management.Data
{
    public class DBService_EJLoganalyst : DBService
    {
        ...
        public DBService_EJLoganalyst(IConfiguration myConfiguration) : base(myConfiguration)
        {
            ...
            FullNameConnection = ConnectString_MySQL.GetValue<string>("ConnectString_NonOutsource:FullNameConnection_baac");
        ...
        public List<Transaction_status_record> GetTransactionStatus()
        {
            ...
            MySqlCommand com = new MySqlCommand();
            com.CommandText = "SELECT Transaction_Status AS TransactionStatus FROM BAACEJLogAnalyst;";
            DataTable tableTemp = _objDb.GetDatatable(com);
        ...
        public LatestUpdated_record GetLatestUpdate()
        {
            MySqlCommand com = new MySqlCommand();
            com.CommandText = "SELECT Update_Date, Update_By FROM BAACEJLogAnalyst ORDER BY Update_Date DESC LIMIT 1;";
            DataTable dt = _objDb.GetDatatable(com);

            // Convert the result to a single record
            List<LatestUpdated_record> result = ConvertDataTableToModel.ConvertDataTable<LatestUpdated_record>(dt);
        ...
        public class EJPatternService : IEJPatternService
        {
            ...
            {
                _connStr = config.GetValue<string>("ConnectString_NonOutsource:FullNameConnection_baac");
            }
            //GSBEJErrorPattern
            public List<string> GetPatterns()
            {
                ...
                lock (_lock)
                {
                    ...
                    conn.Open();
                    using var cmd = new MySqlCommand(
                        "SELECT Pattern FROM BAACEJErrorPattern WHERE IsActive = 1", conn);
