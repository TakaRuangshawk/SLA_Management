...
namespace SLA_Management.Controllers
{
    public class EJLogAnalystController : Controller
    {
        ...
        public EJLogAnalystController(IConfiguration configuration, IEJPatternService patternService)
        {
            ...
            sqlReport = configuration.GetValue<string>("ConnectString_NonOutsource:FullNameConnection_baac");
        ...
        private void ProcessCassetteSegments(string[] lines, string fileName)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                // START CASSETTE 
                if (line.Contains("================CASSETTE INFO================", StringComparison.OrdinalIgnoreCase))
                {
                ...
                if (insideCassette)
                {
                    ...
                    if ((line.Contains("================",StringComparison.OrdinalIgnoreCase) && cassetteBuffer.Length > 100))
                    {
                ...
                if (line.Contains("INSERT CARD", StringComparison.OrdinalIgnoreCase) ||
                    line.Contains("Multi-Task State And NoCard Key Pressed", StringComparison.OrdinalIgnoreCase))
                {
        ...
        {	
            ...
            {
                var m = Regex.Match(firstLine, @"^(?<time>\d{2}:\d{2}:\d{2}\.\d{3})");
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                // ---------------- SEQUENCE NO ----------------                
                if (line.Contains("SEQUENCE NO.", StringComparison.OrdinalIgnoreCase))
                {
                    var match = Regex.Match(line, @"SEQUENCE NO\.\s*:\s*(\d+)", RegexOptions.IgnoreCase);
                    if (match.Success)
                    {
                        tx.SequenceNo = match.Groups[1].Value;
                    }
                }

                // ---------------- AMOUNT ----------------
                if (line.Contains("Amount Entry Field", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        if (decimal.TryParse(raw, out var amt))
                        {
                            // adjust if needed (example: divide by 100)
                            tx.Amount = amt / 100;
                        }
                    }
                }
        ...
        {
            var connStr = _myConfiguration.GetValue<string>("ConnectString_NonOutsource:FullNameConnection_baac");
            using var conn = new MySqlConnection(connStr);
            conn.Open();

            using var checkCmd = new MySqlCommand(@"SELECT COUNT(1) FROM BAACEJLogAnalyst 
