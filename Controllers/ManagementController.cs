...
namespace SLA_Management.Controllers
{
    public class ManagementController : Controller
    {
        ...
        public ManagementController(IConfiguration myConfiguration)
        {
        ...
        public IActionResult InventoryFetchData(string bankcode, string terminalseq, string terminalno, string terminaltype, string connencted, string servicetype, string countertype, string status, string row, string page, string search, string fromdate, string todate, string currentlyinuse)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(_myConfiguration.GetValue<string>("ConnectString_MySQL_FV_"+bankcode+":FullNameConnection")))
            {
                connection.Open();

                // Modify the SQL query to use the 'input' parameter for filtering
                string query = @" SELECT di.DEVICE_ID,di.TERM_SEQ,di.TYPE_ID,di.TERM_ID,di.TERM_NAME,di.TERM_IP,CASE WHEN dsi.CONN_STATUS_ID = 0 THEN 'Online' WHEN dsi.CONN_STATUS_ID is null and di.STATUS = 'no' THEN 'Unknown' ELSE 'Offline' END AS Connected,
                ...
                di.TERM_LOCATION,di.LATITUDE,di.LONGITUDE,di.CONTROL_BY,null as PROVINCE,di.SERVICE_BEGINDATE,
				di.SERVICE_ENDDATE,di.VERSION_AGENT
				FROM device_info di join device_status_info dsi on dsi.TERM_ID = di.TERM_ID
        ...
        private static List<Device_info_record> GetDeviceInfoFeelview(string _bank, IConfiguration _myConfiguration)
        {
            ...
            ConnectMySQL db_mysql = new ConnectMySQL(_myConfiguration.GetValue<string>("ConnectString_MySQL_FV_" + _bank + ":FullNameConnection"));
        ...
        public IActionResult UpdateTerminal(TerminalUpdateModel model)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(_myConfiguration.GetValue<string>("ConnectString_MySQL_FV_" + model.BankCode + ":FullNameConnection")))
                {
                    connection.Open();
